...
class JsonReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        if (options.stream) {
            ...
            this.on('end', () => {
                stats = this.baseReporter.stats
                data = {start:stats.start, end:stats.end, duration:stats._duration};
                ['suites', 'tests', 'passes', 'failures', 'pending'].forEach((item)=> data[item] = stats.counts[item])
                output = ['end', data]
                console.log(JSON.stringify(output));
            })
            this.on('suite:start', (data) => {
                output = ['suite', {title:data.title, fullTitle:data.fullTitle, parent:data.parent}]
                console.log(JSON.stringify(output));
            })
            this.on('test:end', (arg) => {
